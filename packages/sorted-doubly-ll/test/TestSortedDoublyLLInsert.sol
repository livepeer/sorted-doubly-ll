...
contract TestSortedDoublyLLInsert {
    ...
    function beforeEach() public {
        ...
        fixture.initialize(address(this), 3);
    }
    ...
    function test_setMaxSize_decreaseSize() public {
        PublicSortedDoublyLL(address(proxy)).setMaxSize(1);
    ...
    function test_insert_full() public {
        ...
        PublicSortedDoublyLL(address(proxy)).insert(ids[3], keys[3], address(0), address(0));
        bool result = proxy.execute(address(fixture));
        Assert.isFalse(result, "did not revert");
    }
    ...
    function test_insert_containsId() public {
        ...
        PublicSortedDoublyLL(address(proxy)).insert(ids[0], keys[0], address(0), address(0));
        bool result = proxy.execute(address(fixture));
        Assert.isFalse(result, "did not revert");
    }

    function test_insert_null() public {
        PublicSortedDoublyLL(address(proxy)).insert(address(0), keys[0], address(0), address(0));
        bool result = proxy.execute(address(fixture));
        Assert.isFalse(result, "did not revert");
    }

    function test_insert_zeroKey() public {
        PublicSortedDoublyLL(address(proxy)).insert(ids[0], 0, address(0), address(0));
        bool result = proxy.execute(address(fixture));
        Assert.isFalse(result, "did not revert");
    }
}
